...
@Component({
    selector: 'team-together-sp',
    templateUrl: './team-together-sp.component.html',
    styleUrls: ['./team-together-sp.component.css']
})

export class TeamTogetherSPComponent {
    fromYear : number;
    fromMonth : number;
    fromDay : number;
    ...
    onDateRangeChanged(event: IMyDateRangeModel) {
        this.router.navigate(['/team/tsp/' + event.beginDate.year + '/' + event.beginDate.month + '/' + event.beginDate.day + '/to/' + event.endDate.year + '/' + event.endDate.month + '/' + event.endDate.day]);
    ...
    buildChart() {
        ...
        if (simplifiedSet != null) {
            this.chartColumns = [
                ['Time Together'].concat(simplifiedSet[0]),
    ...
    getTeamData(from: DateYMD, to: DateYMD) {
        ...
        dates.forEach(function(d) {
            ...
            self.getTogetherData(index, date);
    ...
    getTogetherData(index: number, date: DateYMD) {
        this.qs.getTeamMetric(this.userService, date).subscribe((res) => {
            this.acquiredData[index] = res.metric.intersect;
