...
export class TeamCommunicationComponent {
    ...
    onDateRangeChanged(event: IMyDateRangeModel) {
        this.router.navigate(['/team/communication/' + event.beginDate.year + '/' + event.beginDate.month + '/' + event.beginDate.day + '/to/' + event.endDate.year + '/' + event.endDate.month + '/' + event.endDate.day]);
    ...
    getTeamData(from: DateYMD, to: DateYMD) {
        ...
        this.qs.currentTeam(this.userService).subscribe((result) => {
            ...
            dates.forEach(function(d) {
                ...
                self.getCommunicationData(self.acquiredMembersObj, index++, {
                    year: d.getFullYear(),
                    month: d.getMonth() + 1,
                    day: d.getDate()
                });
    ...
    getCommunicationData(members: any, index: number, date: DateYMD) {
        this.qs.teamCommunicationData(this.userService, date, false).subscribe((result) => {
            var ret = [];

            result.communication.forEach(function(c) {
                c.member = members[c.member];
        ...
        this.acquiredData.forEach(a => {
            a.forEach(ad => {
                this.chartColumns.forEach((col, idx) => {
                    if (col[0] == ad.member) {
                        this.chartColumns[idx].push(ad.rating);
                    }
                });
            });
        });
